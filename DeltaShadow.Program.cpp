...
namespace DeltaShadow
{
	...
	void Program::DoEvent(const std::shared_ptr<GameObject> &obj, const std::wstring &e)
	{
...
//ORIGINAL LINE: case "event_dragonfire":
	...
	void Program::Event_tankshoot(const std::shared_ptr<GameObject> &obj)
	{
		...
		CreateBullet(obj, L"Tankette", L"Tankette_Bullet", 20, -20, 4, 0, false);
	...
	void Program::Event_gunshoot(const std::shared_ptr<GameObject> &obj)
	{
		...
		CreateBullet(obj, L"Gun", L"Gun_Bullet", 24, -12, 4, 0, true);
		InitSFX(sfx_tanketteshoot);
	}
	...
	void Program::Event_dronshoot0(const std::shared_ptr<GameObject> &obj)
	{
		...
		CreateBullet(obj, L"Dron", L"Dron_Bullet0", 16, 0, 4, 0, false);
		InitSFX(sfx_tanketteshoot);
	}
	...
	void Program::Event_dronshoot1(const std::shared_ptr<GameObject> &obj)
	{
		...
		CreateBullet(obj, L"Dron", L"Dron_Bullet1", 12, 0, 3, 3, false);
		InitSFX(sfx_tanketteshoot);
	}
	...
	void Program::Event_show_hero(const std::shared_ptr<GameObject> &obj)
	{
		...
		InitJAnimation(hero_object, L"JIdle", true);
		InitJIcon(im_none);
		if (obj->Counter == 0)
		{
	...
	void Program::Event_jdoor(const std::shared_ptr<GameObject> &obj)
	{
		...
		StealthSpawn(obj2);
		InitJAnimation(obj2, L"JDoor_Close", true);
	}

	void Program::Event_in_door(const std::shared_ptr<GameObject> &obj)
	{
		std::shared_ptr<GameObject> obj2 = FindDoor();
		InitJAnimation(obj2, L"JDoor_Open", true);
		obj->NoVisible = true;
	...
	void Program::Event_jshoot(const std::shared_ptr<GameObject> &obj)
	{
		...
		o->Animations = FindObjectAnimation(L"Jacqueline");
		InitAnimation(o, L"JBullet", true);
		InitSFX(sfx_stungun);
		o->Counter = 24;
		hero_weapon -= 12;
		if (hero_weapon < 0)
	...
	void Program::Event_lounchrocket(const std::shared_ptr<GameObject> &obj)
	{
		...
		o->Animations = FindObjectAnimation(L"Items");
		InitAnimation(o, L"Rocket0", true);
		o->XAccel = 0;
		o->YAccel = -3;
		o->Counter = 4;
		o->isDamageable = true;
	...
	{
		CreateBullet(obj, L"Jelly", L"Jelly_Bomb", 0, -24, 0, -2, false);
		CreateBullet(obj, L"Jelly", L"Jelly_Bomb", 12, -20, 2, -1, false);
		CreateBullet(obj, L"Jelly", L"Jelly_Bomb", -12, -20, -2, -1, false);
		InitSFX(sfx_tanketteshoot);
	}
	...
	{
		CreateBullet(obj, L"CrabBoss", L"Crab_Claw", 16, -24, 4, 0, false);
		InitSFX(sfx_rocket);
	}

	void Program::Event_crab_upshoot(const std::shared_ptr<GameObject> &obj)
	{
		CreateBullet(obj, L"CrabBoss", L"Crab_Claw1", 8, -32, 3, -3, false);
		InitSFX(sfx_rocket);
	}
	...
	{
		CreateBullet(obj, L"AlienSliderman", L"Slider_Plasma", 0, 8, 0, 3, true);
		CreateBullet(obj, L"AlienSliderman", L"Slider_Plasma", 12, 4, 2, 3, true);
		CreateBullet(obj, L"AlienSliderman", L"Slider_Plasma", -12, 4, -2, 3, true);
		CreateBullet(obj, L"AlienSliderman", L"Slider_Plasma", 16, 0, 3, 2, true);
		CreateBullet(obj, L"AlienSliderman", L"Slider_Plasma", -16, 0, -3, 2, true);
		InitSFX(sfx_tanketteshoot);
	}
	...
	{
		CreateBullet(obj, L"DLCBoss", L"Plasma", 24, -32, 5, 1, false);
		InitSFX(sfx_rocket);
	}

	std::shared_ptr<GameObject> Program::CreateObject(std::uint8_t name, std::int32_t x, std::int32_t y, bool direction, bool actual_object)
	{
		//ищем пустой слот объекта
		for (std::int32_t i = 0; i < game_objects.size(); i++)
		{
	...
	{
		for (std::int32_t i = 0; i < actual_objects.size(); i++)
		{
			std::shared_ptr<GameObject> obj = std::make_shared<GameObject>();
			obj->Name = 0;
	...
	void Program::RemoveActualObject(const std::shared_ptr<GameObject> &obj)
	{
		for (std::int32_t i = 0; i < actual_objects.size(); i++)
		{
			...
			{
				for (std::int32_t j = i; j < actual_objects.size() - 1; j++)
				{
	...
	std::shared_ptr<GameObject> Program::CreateChild(const std::shared_ptr<GameObject> &obj, std::uint8_t name, std::int32_t xoffset, std::int32_t yoffset)
	{
		if (obj->Direction)
		{
			xoffset = -xoffset;
	...
	bool Program::Gravity(const std::shared_ptr<GameObject> &obj)
	{
		ga = 0;
		gb = 0;
	...
	void Program::ChangeDirection(const std::shared_ptr<GameObject> &obj)
	{
		obj->Direction = !obj->Direction;
	}

	bool Program::MoveObjectForward(const std::shared_ptr<GameObject> &obj, std::int32_t speed)
	{
		if (obj->Direction)
		{
			speed = -speed;
	...
	void Program::CreateBullet(const std::shared_ptr<GameObject> &obj, const std::wstring &animations, const std::wstring &anim_name, std::int32_t xoffset, std::int32_t yoffset, std::int32_t xdelta, std::int32_t ydelta, bool damageble)
	{
	...
	void Program::CreateExplosion(const std::shared_ptr<GameObject> &obj, std::int32_t xoffset, std::int32_t yoffset)
	{
		if (ObjectsCount(idExplosion) < 6)
		{
			...
			o->Animations = FindObjectAnimation(L"Items");
			InitAnimation(o, L"Explosion", true);
	...
	void Program::CreateFractions(const std::shared_ptr<GameObject> &obj, std::int32_t num)
	{
		...
		for (std::int32_t i = 0; i < num; i++)
		{
			if (particle_cntr < max)
			{
				...
				o->Animations = FindObjectAnimation(L"Items");
				InitAnimation(o, L"Fraction", true);
	...
	void Program::InitHeroSpike()
	{
		...
		std::wstring anim = L"Nova_Damage";
		if (current_level == 5)
		{
			anim = L"J_Die";
		}
		...
		if (!immortal_mode)
		{
			...
			else
			{
				if (hero_energy != 0)
				{
					...
					InitAnimation(obj, L"JBubble", true);
	...
	std::shared_ptr<GameObject> Program::EnemyDamage(const std::shared_ptr<GameObject> &obj, std::int32_t xdist)
	{
...
std::vector<std::vector<std::vector<Color>>> Program::attr_layer = RectangularVectors::RectangularColorVector(2, 256, 192);

	void Program::InitGEngine()
	{
		screen = std::make_shared<Surface>(256, 192);
		screen_copy = std::make_shared<Surface>(256, 160);
		scroll_screen = std::make_shared<Surface>(256, 160);
		...
		next_palette = std::vector<Color>(256);
		std::shared_ptr<Stream> input;
		if (external_resource)
		{
			input = File::Open(LR"(../../../../../res/NextPalette.bin)", FileMode::Open);
		}
		else
		{
			input = std::make_shared<MemoryStream>(Properties->Resources::getNextPalette());
		}
		std::shared_ptr<BinaryReader> f = std::make_shared<BinaryReader>(input);
		for (std::int32_t i = 0; i < 256; i++)
		{
	...
	System::Drawing::Color Program::GetColor(std::int32_t zxcolor)
	{
	...
	void Program::FillAttrRect(std::int32_t x, std::int32_t y, std::int32_t width, std::int32_t height, std::uint8_t attr)
	{
		...
		screen->Fill(Rectangle(x * 8, y * 8, width * 8, height * 8), GetAttrColor(attr, false));
	}
	...
	void Program::DrawRoom()
	{
		...
		for (std::int32_t j = 0; j < 10; j++)
		{
			for (std::int32_t i = 0; i < 16; i++)
			{
				...
				{
					screen->Blit(Tileset[tiles_mode][Room[i, j]], Point(i * 16, j * 16));
					if (tiles_mode == 0)
					{
						for (std::int32_t ya = 0; ya < 16; ya++)
						{
							...
							{
								attr_layer[0][i * 16 + xa][j * 16 + ya] = TilesetAttr[0][Room[i, j]][xa][ya];
								attr_layer[1][i * 16 + xa][j * 16 + ya] = TilesetAttr[1][Room[i, j]][xa][ya];
							}
						}
					}
	...
	void Program::DrawSymbolObject(const std::shared_ptr<GameObject> &obj)
	{
		...
		else
		{
			...
			screen->Blit(next_mini_powerups, Point(i, j), Rectangle(obj->XAccel * 8, 0, 8, 8));
			i = i / 8;
			j = j / 8;
		}
		screen_refresh_tab[i / 2][j / 2] = true;
	...
	{
		std::vector<std::shared_ptr<ObjectAnimation::CompositionElement>> composition = obj->Animations[sprite_mode]->Animations[obj->CurrentAnimation].Frames[obj->Frame].Composition;
	...
	void Program::ScrollRoom()
	{
		...
		screen->ClipRectangle = Rectangle(0, 0, 256, 160);
		...
		screen->ClipRectangle = Rectangle(0, 0, 256, 192);
	...
	{
		scroll_direction = 0;
		std::wstring name = levels_names[num];

		std::shared_ptr<Stream> str;
		std::wstring path = LR"(../../../../../res/locations/)" + name + L".loc";
		if (external_resource)
		{
		...
		{
			name = StringHelper::replace(name, L"-", L"_");
			name = StringHelper::replace(name, L" ", L"_");
			str = std::make_shared<MemoryStream>(static_cast<std::vector<std::uint8_t>>(Properties->Resources::getResourceManager()->GetObject(name)));
		}
		std::shared_ptr<BinaryReader> f = std::make_shared<BinaryReader>(str);

		//загрузка локации
		std::int32_t v = f->ReadByte();
		if (v != 0)
		{
			this->Quit(shared_from_this(), std::make_shared<QuitEventArgs>());
		}
		f->ReadByte();
...
//ORIGINAL LINE: Loc = new int[LocWidth, LocHeight];
		Loc = RectangularVectors::RectangularStd::int32_tVector(LocWidth, LocHeight);
		...
		//загрузка тайлсетов
//C# TO C++ CONVERTER NOTE: The following call to the 'RectangularVectors' helper class reproduces the rectangular array initialization that is automatic in C#:
//ORIGINAL LINE: current_tilesets = new byte[2, 256, 37];
		current_tilesets = RectangularVectors::RectangularStd::uint8_tVector(2, 256, 37);
...
//C# TO C++ CONVERTER NOTE: The following call to the 'RectangularVectors' helper class reproduces the rectangular array initialization that is automatic in C#:
//ORIGINAL LINE: Color[,] tileset256 = new Color[256, 256];
		std::vector<std::vector<Color>> tileset256 = RectangularVectors::RectangularColorVector(256, 256);
		if (f->PeekChar() != -1)
		{
		...
		for (std::int32_t i = 0; i < 256; i++)
		{
			...
			for (std::int32_t y = 0; y < 16; y++)
			{
				for (std::int32_t x = 0; x < 16; x++)
				{
					...
					Color c = ((p << (x % 8)) & 0x80) == 0 ? paper : ink;
	...
	void Program::SetBorder(Color c)
	{
		Video::Screen->Fill(c);
	}

std::wstring Program::language = L"English";
...
std::int32_t Program::tiles_mode = zx_classic_mode;
	...
	void Program::MainInit()
	{
		...
		objects_names.emplace(L"none", 0);
		objects_names.emplace(L"Nova", idNova);
		objects_names.emplace(L"Dust", idDust);
		objects_names.emplace(L"Explosion", idExplosion);
		objects_names.emplace(L"RocketSmoke", idRocketSmoke);
		objects_names.emplace(L"MultiExplosions", idMultiExplosions);
		objects_names.emplace(L"Blower", idBlower);
		objects_names.emplace(L"Bubble", idBubble);
		objects_names.emplace(L"Bumerang", idBumerang);
		objects_names.emplace(L"Wave", idWave);
		objects_names.emplace(L"Battery", idBattery);
		objects_names.emplace(L"Heart", idHeart);
		objects_names.emplace(L"Powerup", idPowerup);
		objects_names.emplace(L"MiniBattery", idMiniBattery);
		objects_names.emplace(L"MiniHeart", idMiniHeart);
		objects_names.emplace(L"MiniPowerup", idMiniPowerup);
		objects_names.emplace(L"Suit", idSuit);
		objects_names.emplace(L"Star", idStar);
		objects_names.emplace(L"Platform", idPlatform);
		objects_names.emplace(L"Steam", idSteam);
		objects_names.emplace(L"Fraction", idFraction);
		objects_names.emplace(L"Door", idDoor);
		objects_names.emplace(L"Tankette", idTankette);
		objects_names.emplace(L"Dron", idDron);
		objects_names.emplace(L"Bullet", idBullet);
		objects_names.emplace(L"Spider", idSpider);
		objects_names.emplace(L"Piranha", idPiranha);
		objects_names.emplace(L"BigPiranha", idBigPiranha);
		objects_names.emplace(L"Jelly", idJelly);
		objects_names.emplace(L"SpiderNest", idSpiderNest);
		objects_names.emplace(L"Bug", idBug);
		objects_names.emplace(L"Gun", idGun);
		objects_names.emplace(L"Dragonfly", idDragonfly);
		objects_names.emplace(L"FighterBomber", idFighterBomber);
		objects_names.emplace(L"DronBomber", idDronBomber);
		objects_names.emplace(L"AlienClawBottom", idAlienClawBottom);
		objects_names.emplace(L"AlienClawTop", idAlienClawTop);
		objects_names.emplace(L"AlienWorm", idAlienWorm);
		objects_names.emplace(L"Slime", idSlime);
		objects_names.emplace(L"SonusVenator", idSonusVenator);
		objects_names.emplace(L"BigSpider", idBigSpider);
		objects_names.emplace(L"Craken", idCraken);
		objects_names.emplace(L"Moth", idMoth);
		objects_names.emplace(L"Rocketman", idRocketman);
		objects_names.emplace(L"CrabBoss", idCrabBoss);
		objects_names.emplace(L"FlyBoss", idFlyBoss);
		objects_names.emplace(L"AlienPet", idAlienPet);
		objects_names.emplace(L"AlienSliderman", idAlienSliderman);
		objects_names.emplace(L"Megabrain", idMegabrain);
		objects_names.emplace(L"Rocket", idRocket);
		objects_names.emplace(L"MiniBrain", idMiniBrain);
		objects_names.emplace(L"Bomb", idBomb);
		objects_names.emplace(L"FlyBomb", idFlyBomb);
		objects_names.emplace(L"WaterFun", idWaterFun);
		objects_names.emplace(L"BubbleGenerator", idBubbleGenerator);
		objects_names.emplace(L"Jacqueline", idJacqueline);
		objects_names.emplace(L"JLift", idJLift);
		objects_names.emplace(L"JDoor", idJDoor);
		objects_names.emplace(L"JTerminal", idJTerminal);
		objects_names.emplace(L"JPolice1", idJPolice1);
		objects_names.emplace(L"JPolice2", idJPolice2);
		objects_names.emplace(L"JPolice3", idJPolice3);
		objects_names.emplace(L"JOffice1", idJOffice1);
		objects_names.emplace(L"JOffice2", idJOffice2);
		objects_names.emplace(L"JOffice3", idJOffice3);
		objects_names.emplace(L"NovaBike", idNovaBike);
		objects_names.emplace(L"JDJump", idJDJump);
		objects_names.emplace(L"JGun", idJGun);
		objects_names.emplace(L"JShield", idJShield);
		objects_names.emplace(L"Dragon", idDragon);

		levels_names.emplace(0, L"level 1-1");
		levels_names.emplace(1, L"level 1-2");
		levels_names.emplace(2, L"level 1-3");
		levels_names.emplace(3, L"level 1-4");
		levels_names.emplace(4, L"dragon 1");
		levels_names.emplace(5, L"boss 1");
		levels_names.emplace(6, L"level 2-1");
		levels_names.emplace(7, L"level 2-2");
		levels_names.emplace(8, L"level 2-3");
		levels_names.emplace(9, L"level 2-4");
		levels_names.emplace(10, L"dragon 2");
		levels_names.emplace(11, L"boss 2");
		levels_names.emplace(12, L"level 3-1");
		levels_names.emplace(13, L"level 3-2");
		levels_names.emplace(14, L"level 3-3");
		levels_names.emplace(15, L"level 3-4");
		levels_names.emplace(16, L"dragon 3");
		levels_names.emplace(17, L"boss 3");
		levels_names.emplace(18, L"level 4-1");
		levels_names.emplace(19, L"level 4-2");
		levels_names.emplace(20, L"level 4-3");
		levels_names.emplace(21, L"level 4-4");
		levels_names.emplace(22, L"level 4-5");
		levels_names.emplace(23, L"level 4-6");
		levels_names.emplace(24, L"level 4-7");
		levels_names.emplace(25, L"level 4-8");
		levels_names.emplace(26, L"subboss 1");
		levels_names.emplace(27, L"subboss 2");
		levels_names.emplace(28, L"subboss 3");
		levels_names.emplace(29, L"subboss 4");
		levels_names.emplace(30, L"subboss 5");
		levels_names.emplace(31, L"subboss 6");
		levels_names.emplace(32, L"finalboss 0");
		levels_names.emplace(33, L"finalboss 1");
		levels_names.emplace(34, L"finalboss 2");
		levels_names.emplace(35, L"Jacqueline");
		levels_names.emplace(36, L"DLC");
		levels_names.emplace(37, L"DLCBoss");

		music_names.emplace(0, L"1-1");
		music_names.emplace(1, L"1-2");
		music_names.emplace(2, L"1-3");
		music_names.emplace(3, L"2-1");
		music_names.emplace(4, L"2-2");
		music_names.emplace(5, L"2-3");
		music_names.emplace(6, L"3-1");
		music_names.emplace(7, L"3-2");
		music_names.emplace(8, L"3-3");
		music_names.emplace(9, L"4-1");
		music_names.emplace(10, L"boss");
		music_names.emplace(11, L"finalboss");
		music_names.emplace(12, L"stealth_1");
		music_names.emplace(13, L"stealth_2");
		music_names.emplace(14, L"stealth_3");
		music_names.emplace(15, L"race");

		music_names.emplace(247, L"5-1");
		music_names.emplace(248, L"game_over");
		music_names.emplace(249, L"Secret boss");
		music_names.emplace(250, L"finalcut");
		music_names.emplace(251, L"intro");
		music_names.emplace(252, L"motorbike");
		music_names.emplace(253, L"menu");
		music_names.emplace(254, L"clip");
...
std::wstring Program::current_save_version = L"1.5";

	void Program::SaveProgress()
	{
		std::shared_ptr<BinaryWriter> f = std::make_shared<BinaryWriter>(File::Create(Environment::GetFolderPath(Environment::SpecialFolder::ApplicationData) + LR"(\blade_project.saves)"));
	...
	void Program::LoadProgress()
	{
		...
		std::wstring path = Environment::GetFolderPath(Environment::SpecialFolder::ApplicationData) + LR"(\blade_project.saves)";
		if (!File::Exists(path))
		{
			return;
		}

		std::shared_ptr<BinaryReader> f = std::make_shared<BinaryReader>(File::Open(path, FileMode::Open));

		//проверяем версию
		std::wstring ver = f->ReadString();
	...
	void Program::StartTimer_Tick(const std::shared_ptr<void> &sender, const std::shared_ptr<EventArgs> &e)
	{
	...
	void Program::DrawDebugMenu()
	{
		...
		invert_print = n == menu_pos;
		std::wstring s = L"Classic";
		if (sprite_mode == zx_next_mode)
		{
			s = L"Next";
		}
		PrintString32(0, 16, L"Sprites: " + s);
		n++;
		invert_print = n == menu_pos;
		s = L"Classic";
		...
		PrintString32(0, 19, L"All suits enabled: " + s);
		n++;
		invert_print = n == menu_pos;
		if (immortal_mode)
		{
		...
		PrintString32(0, 20, L"Immortal mode: " + s);
		invert_print = false;

		PrintString32(0, 22, L"ESC. Return to this menu");
		PrintString32(0, 23, L"+/- change screen size");
	}
	...
	void Program::InitGameOver()
	{
		...
		MuteGameMusic();
		ClearScreen();
		statusbar = InitScreensProject(L"bar");
	...
	void Program::InitSelectLevel()
	{
		...
		MuteAllMusic();
		LoadTexts();
		select_level_screens = InitScreensProject(L"select_level");
	...
	void Program::InitMainMenu(bool hard_init)
	{
		...
		if (hard_init)
		{
			ClearScreen();
			MuteMenuMusic();
			main_menu_scr = InitScreensProject(L"main_menu");
			DrawScreen(main_menu_scr, 0);
	...
	void Program::UpdateMainMenu()
	{
		...
		if (GetPressedKey(Key_Fire + Key_Jump + Key_Select + Key_Pause))
		{
			...
			if (main_menu_pos == 0 || GetPressedKey(Key_Pause))
			{
				...
				{
					statusbar = InitScreensProject(L"bar");
					select_menu = InitScreensProject(L"select_suit");
				...
				{
					statusbar = InitScreensProject(L"j_bar");
				...
				{
					race_imgs = InitScreensProject(L"race");
					game_mode = race_mode;
					game_cicle_counter = 0;
					ClearScreen();
	...
	void Program::UpdateSelectMessage()
	{
		if ((interrupt_counter & 3) == 0 || GetPressedKey(255))
		{
			...
			if (GetPressedKey(Key_Fire + Key_Jump + Key_Select) && n < stars_num)
			{
				...
				std::shared_ptr<Sprite> spr = FindSprite(main_menu_scr, std::to_wstring(n + 1));
...
//ORIGINAL LINE: case "Russian":
					else if (language == L"Russian")
					{
							language = L"Polish";
					}
//ORIGINAL LINE: case "Polish":
					else if (language == L"Polish")
					{
							language = L"Spanish";
					}
//ORIGINAL LINE: case "Spanish":
					else if (language == L"Spanish")
					{
							language = L"English";
					}
					LoadTexts();
	...
	void Program::PrintDemoVersion()
	{
		...
		PrintString32(0, 0, L"Demo version");
		PrintString32(20, 23, L"Demo version");
	}

	void Program::LoadAnimations(const std::wstring &name)
	{
		LoadAnimationsProject(name + L"Classic.scproj");
		LoadAnimationsProject(name + L"Next.scproj");
	}

	void Program::LoadAnimationsProject(const std::wstring &name)
	{
		std::shared_ptr<ObjectAnimation> anim = std::make_shared<ObjectAnimation>();
		anim->Name = name;

		std::shared_ptr<Stream> str;
		std::wstring path = LR"(../../../../../res/animations/)" + name;
		if (external_resource)
		{
			str = File::Open(path, FileMode::Open);
		}
		...
		{
			str = std::make_shared<MemoryStream>(static_cast<std::vector<std::uint8_t>>(Properties->Resources::getResourceManager()->GetObject(name.substr(0, name.length() - 7))));
		}
		std::shared_ptr<BinaryReader> f = std::make_shared<BinaryReader>(str);
		...
		for (std::int32_t i = 0; i < spr_count; i++)
		{
			...
			{
//C# TO C++ CONVERTER NOTE: The following call to the 'RectangularVectors' helper class reproduces the rectangular array initialization that is automatic in C#:
//ORIGINAL LINE: s.ClassicLayer = new int[s.Width, s.Height];
				s->ClassicLayer = RectangularVectors::RectangularStd::int32_tVector(s->Width, s->Height);
			...
			{
//C# TO C++ CONVERTER NOTE: The following call to the 'RectangularVectors' helper class reproduces the rectangular array initialization that is automatic in C#:
//ORIGINAL LINE: s.NextLayer = new Color[s.Width, s.Height];
				s->NextLayer = RectangularVectors::RectangularColorVector(s->Width, s->Height);
				for (std::int32_t y = 0; y < s->Height; y++)
				{
					for (std::int32_t x = 0; x < s->Width; x++)
					{
	...
	std::shared_ptr<ObjectAnimation::Sprite> Program::FindSprite(const std::wstring &name, const std::shared_ptr<ObjectAnimation> &anim)
	{
	...
	void Program::InitAnimation(const std::shared_ptr<GameObject> &obj, const std::wstring &name, bool hard_enable)
	{
		std::int32_t i = 0;
		for (auto a : obj->Animations[sprite_mode]->Animations)
		{
	...
	void Program::PlayAnimation(const std::shared_ptr<GameObject> &obj)
	{
		...
		obj->Frame++;
		if (obj->Frame == obj->Animations[sprite_mode]->Animations[obj->CurrentAnimation].Frames->Count)
		{
		...
		std::wstring e = obj->Animations[sprite_mode]->Animations[obj->CurrentAnimation].Frames[obj->Frame].Event;
		GetObjSize(obj);
	...
	void Program::GetObjSize(const std::shared_ptr<GameObject> &obj)
	{
	...
	std::vector<std::shared_ptr<ObjectAnimation>> Program::FindObjectAnimation(const std::wstring &name)
	{
		std::vector<std::shared_ptr<ObjectAnimation>> anim(2);
		for (auto a : ObjectAnimations)
		{
			if (a->Name == name + L"Classic.scproj")
			{
				anim[0] = a;
			}
		}
	...
	void Program::InitPlatforming(bool boss)
	{
		...
		LoadAnimations(L"Nova");
		LoadAnimations(L"Suit");
		LoadAnimations(L"Items");
		LoadAnimations(L"Dron");
		LoadAnimations(L"Tankette");
		LoadAnimations(L"Spider");
		LoadAnimations(L"Piranha");
		LoadAnimations(L"Jelly");
		LoadAnimations(L"Bug");
		LoadAnimations(L"DronBomber");
		LoadAnimations(L"Gun");
		LoadAnimations(L"Dragonfly");
		LoadAnimations(L"FighterBomber");
		LoadAnimations(L"AlienClaw");
		LoadAnimations(L"AlienWorm");
		LoadAnimations(L"Slime");
		LoadAnimations(L"SonusVenator");

		LoadAnimations(L"BigSpider");
		LoadAnimations(L"Craken");
		LoadAnimations(L"Moth");
		LoadAnimations(L"Rocketman");
		LoadAnimations(L"CrabBoss");
		LoadAnimations(L"FlyBoss");
		LoadAnimations(L"AlienPet");
		LoadAnimations(L"AlienSliderman");
		LoadAnimations(L"Megabrain");

		LoadAnimations(L"DLCJacqueline");
		LoadAnimations(L"DLCBoss");
	...
	void Program::InitLocation(bool boss)
	{
		...
		if (current_level != 5)
		{
			...
			hero_object->Animations = FindObjectAnimation(L"Nova");
			InitAnimation(hero_object, L"Nova_Idle", true);
		...
		//DLC с Жаклин
			...
			hero_object->Animations = FindObjectAnimation(L"DLCJacqueline");
			InitAnimation(hero_object, L"JIdle", true);
			hero_object->Priority = normal_priority;
			hero_object->haveGravity = true;
			InitRoom(true);
	...
	void Program::Nova(const std::shared_ptr<GameObject> &obj)
	{
		...
		if (!secret_boss_enabled)
		{
			...
			if (on_wall)
			{
				anim = L"Nova_WallIdle";
			}

			//на потолке
			if (on_roof)
			{
				anim = L"Nova_RoofIdle";
			}
			...
			else
			{
				if (hero_die && !on_air)
				{
					InitAnimation(obj, L"Nova_Die", false);
				}
				return;
			}
			...
			if (slide_cntr != 0)
			{
				...
				if (graphics_quality == 2 && (~slide_cntr & 7) == 0 && GetProperty(obj, 0, obj->Height) != isWater)
				{
					...
					o->Animations = FindObjectAnimation(L"Nova");
					o->Fixed = true;
					InitAnimation(o, L"Nova_Dust", true);
				}
			}
				...
				if (GetKey(Key_Up))
				{
					...
					else if (on_wall && !GetKey(Key_Left + Key_Right))
					{
						...
						else if (GetProperty(obj, obj->Width + 4, obj->Height - 26) == isEmpty || GetProperty(obj, obj->Width + 4, obj->Height - 26) == isWater)
						{
							...
							InitAnimation(obj, L"Nova_WallUp", false);
							return;
						}
					...
					if (!on_ladder && !is_fly)
					{
						...
						else if (on_wall && GetProperty(obj, 10, 0) != isSpike)
						{
							...
							else if (GetProperty(obj, 10, -28) != isFloor && GetProperty(obj, 10, -12) != isFloor && GetProperty(obj, 10, 4) != isFloor)
							{
								...
								obj->X += q;
								InitAnimation(obj, L"Nova_CornerRoof", false);
							...
							else if (on_roof)
							{
								...
								if (q != isFloor)
								{
									if (q != isSpike)
									{
										...
										InitAnimation(obj, L"Nova_CornerWall", false);
										return;
									}
									else
									{
		...
		if (attack_cntr != 0)
		{
			...
			if (on_air)
			{
				anim = L"Nova_JumpAttack";
			}
			if (is_crouch)
			{
				anim = L"Nova_CrouchAttack";
				yw = -10;
			}
			if (on_ladder || on_roof)
			{
				anim = L"Nova_LadderAttack";
			}
			if (GetKey(Key_Up) && !GetKey(Key_Left + Key_Right))
			{
				anim = L"Nova_UpAttack";
				xw = 0;
				yw = -33;
				xd = 0;
				yd = -5;
			...
			if (GetKey(Key_Down) && !GetKey(Key_Left + Key_Right) && (on_air || on_ladder || on_wall || on_roof))
			{
				if (!on_roof)
				{
					anim = L"Nova_DownAttack";
				}
				else
				{
					anim = L"Nova_RoofAttack";
				}
				xw = 0;
				yw = 0;
				xd = 0;
			...
			if (attack_cntr == 3)
			{
				...
				if (current_suit == 0)
				{
					...
					o->YAccel = 16;
					o->Animations = FindObjectAnimation(L"Nova");
					InitAnimation(o, L"Nova_Bumerang", true);
				...
				else
				{
					...
					o->YTarget = yd;
					o->Counter = 24;
					o->Animations = FindObjectAnimation(L"Suit");
					o->Fixed = true;
					InitAnimation(o, L"Nova_Wave" + std::to_wstring(dir), true);
					InitSFX(sfx_wave);
				}
			...
			anim += std::to_wstring(attack_cntr / 3);
			attack_cntr--;
		}
		...
		if (GetXDistance(obj) != 255 && GetYDistance(obj) != 159)
		{
			for (auto o : actual_objects)
			{
				...
				if (have_garvity && std::abs(o->Y - obj->Y) < 8 && o->Y >= obj->Y && obj->Y >= o->Y + o->Height && obj->X + obj->Width >= o->X - o->Width && o->X + o->Width >= obj->X - obj->Width)
				{
					...
					{
						InitAnimation(obj, L"Platform_Move", false);

						obj->XAccel = 255;
						obj->Fixed = false;

						//поиск и уничтожение парной платформы
						for (auto obj2 : game_objects)
						{
	...
	void Program::Steam(const std::shared_ptr<GameObject> &obj)
	{
		if (obj->Counter == 0)
		{
			...
			InitAnimation(obj, L"Steam", true);
			InitSFX(sfx_steam);
			obj->NoVisible = false;
		}
		else
	...
	void Program::Door(const std::shared_ptr<GameObject> &obj)
	{
		if (room_cntr == 1 && obj->Counter != 0)
		{
			...
			if (obj->Counter == 0)
			{
				InitAnimation(obj, L"Door_Open", false);
		...
		if (GetKey(Key_Up) == true && d < 16 && GetYDistance(obj) < 3)
		{
			...
			{
				InitAnimation(obj, L"Door_Open", true);
				InitAnimation(hero_object, L"Nova_Idle", true);
				disable_control = true;
				is_fly = false;
				MuteAllMusic();
	...
	void Program::Tankette(const std::shared_ptr<GameObject> &obj)
	{
		...
		if (!obj->isDamageable)
		{
			...
			{
				TurnObject(obj);
				InitAnimation(obj, L"Tankette_Turn", false);
				obj->NoInterruptAnim = true;
			}
		...
		std::wstring anim = L"Tankette_Move";
		bool hard_anim = false;
		...
		if (can_shoot || obj->Counter != 0)
		{
			...
			{
				obj->Status++;
				anim = L"Tankette_Activate";
				obj->Counter = 4;
			}
			else
			{
				anim = L"Tankette_Shoot";
				hard_anim = true;
			}
			obj->NoInterruptAnim = true;
		}
	...
	void Program::Dron(const std::shared_ptr<GameObject> &obj)
	{
		...
		if (!obj->isDamageable && !obj->haveGravity)
		{
			...
			obj->haveGravity = true;
			obj->isDamageable = true;
			InitAnimation(obj, L"Dron_Idle", true);
			return;
		}
	...
	void Program::Piranha(const std::shared_ptr<GameObject> &obj)
	{
		...
		std::wstring anim = L"SmallPiranha_Swim";
		...
		{
			anim = L"SmallPiranha_Attack";
			if (obj->Name == idBigPiranha)
			{
				anim = L"Piranha_Attack";
			}
			speed = 4;
	...
	void Program::SpiderNest(const std::shared_ptr<GameObject> &obj)
	{
		...
		if (c < 3 && obj->Counter == 0 && hero_life != 0)
		{
			...
			o->Animations = FindObjectAnimation(L"Spider");
			InitAnimation(o, L"Spider_Run", true);
	...
	void Program::FlyEnemy(const std::shared_ptr<GameObject> &obj)
	{
		...
		if (obj->Name == idDragonfly)
		{
			...
			if (obj->Counter != 0)
			{
				...
				if (obj->Counter == 0)
				{
					...
					o->Animations = FindObjectAnimation(L"Items");
					InitAnimation(o, L"Rocket2", true);
					o->XAccel = 3;
					o->Counter = 4;
					o->isDamageable = true;
					o->Fixed = true;
		...
		else
		{
			...
			else if (GetXDistance(obj) < 8 && obj->Y < hero_object->Y && TestShoot(obj, 0, 0, 1))
			{
				...
				o->Fixed = true;
				o->Animations = FindObjectAnimation(L"DronBomber");
				InitAnimation(o, L"DronBomber_Bomb", true);
				InitSFX(sfx_gotonthewall);
			}
		}
	...
	void Program::AlienWorm(const std::shared_ptr<GameObject> &obj)
	{
		...
		if (obj->Status == 1)
		{
			...
			{
				obj->Status++;
				anim = L"AlienWorm_MoveDown";
				obj->Y += 16;
			}
			else
			{
				anim = L"AlienWorm_Idle";
			}
		}
		else if (obj->Status == 2)
		{
	...
	void Program::Slime(const std::shared_ptr<GameObject> &obj)
	{
		...
		if (!obj->isDamageable)
		{
			...
			if (obj->Life != 0)
			{
				for (std::int32_t i = 0; i < 2; i++)
				{
					...
					o->Animations = FindObjectAnimation(L"Slime");
					o->Life = obj->Life - 1;
					InitAnimation(o, L"Walk" + std::to_wstring(o->Life), true);
					o->Direction = i == 0;
					o->YAccel = -17;
					o->isDamageable = true;
					o->UndeadCounter = 15;
		...
		if (obj->Status == 0)
		{
			if (!on_air)
			{
				anim = L"Walk" + std::to_wstring(obj->Life);
			}
			...
			{
				anim = L"Jump" + std::to_wstring(obj->Life);
				obj->Status++;
				obj->Counter = 0;
			}
		}
		...
		else if (obj->Status == 1)
		{
			...
			{
				anim = L"Fall" + std::to_wstring(obj->Life);
				obj->NoInterruptAnim = true;
				obj->Status--;
				TurnObject(obj);
			}
	...
	void Program::SonusVenator(const std::shared_ptr<GameObject> &obj)
	{
		...
		std::wstring anim = L"Venator_Idle";
		bool hard_anim = false;
	...
	void Program::BigSpider(const std::shared_ptr<GameObject> &obj)
	{
		...
		if (obj->Status < 3)
		{
			...
			{
				anim = L"BigSpider_RunFloor";
				if (!MoveObjectForward(obj, 4))
				{
					obj->Status++;
					obj->Counter = 4;
					obj->haveGravity = false;
					anim = L"BigSpider_MoveOnWall";
				}
			}
			else if (obj->Status == 1)
			{
				anim = L"BigSpider_RunWall";
				obj->Y -= 2;
				if (GetProperty(obj, 0, -44) == isFloor)
				{
					...
					ChangeDirection(obj);
					obj->Counter = 4;
					anim = L"BigSpider_MoveOnRoof";
				}
			}
			else if (obj->Status == 2)
			{
				anim = L"BigSpider_RunRoof";

				if (!MoveObjectForward(obj, 4) || GetXDistance(obj) < 32)
				{
					obj->haveGravity = true;
					anim = L"BigSpider_Attack";
					if (GetXDistance(obj) >= 32)
					{
						ChangeDirection(obj);
					}
	...
	void Program::Craken(const std::shared_ptr<GameObject> &obj)
	{
		...
		Gravity(obj);
		std::wstring anim = L"Craken_Idle";
		...
		else
		{
			...
			else
			{
				...
				{
					std::shared_ptr<GameObject> o = CreateChild(obj, idBomb, 0, 0);
					o->Animations = FindObjectAnimation(L"Craken");
					InitAnimation(o, L"Craken_Bomb", true);
					o->YAccel = -15;
					o->Fixed = true;
					InitSFX(sfx_gotonthewall);
				}
	...
	void Program::Moth(const std::shared_ptr<GameObject> &obj)
	{
		if (obj->Counter != 0)
		{
			...
			if (obj->Counter == 0)
			{
				InitAnimation(obj, L"Moth_Fly", true);
		...
		if (obj->Status == 0)
		{
			if (MoveObjectForward(obj, 3))
			{
				...
				if (obj->XAccel != 0 && obj->YAccel == 0 && ObjectsCount(idFlyBomb) < 2)
				{
					...
					o->Animations = FindObjectAnimation(L"Moth");
					InitAnimation(o, L"Moth_Bomb", true);
					o->isDamageable = true;
					obj->XAccel--;
					obj->YAccel = 48;
					InitSFX(sfx_gotonthewall);
	...
	void Program::Rocketman(const std::shared_ptr<GameObject> &obj)
	{
		...
		std::wstring anim = L"Rocketman_Idle";
		if (on_air)
		{
			anim = L"Rocketman_Fall";
		}

		if (obj->Status == 0 && !on_air && GetProperty(obj, -16, -64) != isFloor && GetXDistance(obj) >= 33)
		{
			anim = L"Rocketman_Fire";
			obj->NoInterruptAnim = true;
			obj->Status++;
		}
	...
	void Program::FlyBoss(const std::shared_ptr<GameObject> &obj)
	{
		...
		std::wstring anim = L"FlyBoss_Idle";
		bool hard_anim = false;
		bool on_air = false;
		if (obj->haveGravity)
		{
	...
	void Program::AlienSliderman(const std::shared_ptr<GameObject> &obj)
	{
		...
		if (obj->Status != 3)
		{
			...
			if (obj->XAccel >= 64)
			{
				...
				{
					anim = L"Slider_Attack";
				}
				else
				{
					TurnObject(obj);
					anim = L"Slider_Magnit";
					InitSFX(sfx_steam);
				}
				obj->NoInterruptAnim = true;
			}
	...
	void Program::Megabrain(const std::shared_ptr<GameObject> &obj)
	{
		if (!secret_boss_enabled)
		{
			...
			if (!obj->isDamageable)
			{
				...
				InitAnimation(obj, L"Megabrain_" + std::to_wstring(n), false);
			...
			if (obj->Counter != 0)
			{
				...
				if (obj->Counter == 0)
				{
					...
					if (ObjectsCount(idMiniBrain) < 3)
					{
						...
						o->Animations = FindObjectAnimation(L"Megabrain");
						InitAnimation(o, L"MiniBrain", true);
						o->isDamageable = true;
						InitSFX(sfx_gotonthewall);
					}
				}
		...
		else
		{
			...
			InitAnimation(obj, L"Megabrain_4", false);
		}
	}
	...
	void Program::Rocket(const std::shared_ptr<GameObject> &obj)
	{
		...
		else
		{
			...
			InitAnimation(obj, L"Rocket" + std::to_wstring(a), false);
...
Gamepad Program::x_input_gamepad;
	...
	Program::Program()
	{
		...
		Video::WindowCaption = L"Delta's Shadow";
		MainInit();
	...
	{
		StartKempston();
		Events::Quit += std::make_shared<EventHandler<std::shared_ptr<QuitEventArgs>>>(Quit);
		Events::Tick += std::make_shared<EventHandler<std::shared_ptr<TickEventArgs>>>(Update);
		Events::TargetFps = 50;
		Events::KeyboardDown += std::make_shared<EventHandler<std::shared_ptr<KeyboardEventArgs>>>(KeyboardDown);
		Events::KeyboardUp += std::make_shared<EventHandler<std::shared_ptr<KeyboardEventArgs>>>(KeyboardUp);
		Events::MouseMotion += std::make_shared<EventHandler<std::shared_ptr<MouseMotionEventArgs>>>(MouseMotion);
		Events::VideoResize += std::make_shared<EventHandler<std::shared_ptr<VideoResizeEventArgs>>>(VideoResize);
		Events::Run();
	}

	void Program::Quit(const std::shared_ptr<void> &sender, const std::shared_ptr<QuitEventArgs> &e)
	{
...
std::int32_t Program::y_screen_home = 16;
	...
	void Program::Update(const std::shared_ptr<void> &sender, const std::shared_ptr<TickEventArgs> &e)
	{
		...
		if (full_screen_mode)
		{
			Size win = System::Windows::Forms::Screen::PrimaryScreen::Bounds::Size;
			std::int32_t sx = win.Width / 256;
			std::int32_t sy = win.Height / 192;
			s = static_cast<double>(std::min(sx, sy));
		}

		std::shared_ptr<Surface> sur = screen->CreateScaledSurface(s);
		Video::Screen->Blit(sur, Point(static_cast<std::int32_t>(x_screen_home * s), static_cast<std::int32_t>(y_screen_home * s)));
		Video::Screen->Update();
		sur->Dispose();
	}

	void Program::MouseMotion(const std::shared_ptr<void> &sender, const std::shared_ptr<MouseMotionEventArgs> &e)
	{
		if (show_cursor_timer != 0)
		{
			return;
	...
	void Program::KeyboardDown(const std::shared_ptr<void> &sender, const std::shared_ptr<KeyboardEventArgs> &e)
	{
		Key key = e->Key;
		...
		pressed_key = key;
		if (pressed_key_symbol > static_cast<std::int32_t>(L'9'))
		{
//C# TO C++ CONVERTER TODO TASK: There is no C++ equivalent to 'ToString':
			pressed_key_symbol = static_cast<std::int32_t>(key.ToString().ToUpper()[0]);
		}

		if ((key >= Key::Zero && key <= Key::Nine) || (key >= Key::A && key <= Key::Z) || key == Key::Return || key == Key::LeftShift || key == Key::LeftControl || key == Key::RightShift || key == Key::RightControl || key == Key::Space || key == Key::Escape)
		{
			switch (key)
			{
				case Key::Return:
		...
		{
			Quit(shared_from_this(), std::make_shared<QuitEventArgs>());
		}
	}

	void Program::KeyboardUp(const std::shared_ptr<void> &sender, const std::shared_ptr<KeyboardEventArgs> &e)
	{
		if (e->Key == KeysMap[0])
		{
			PressedKeysStatus = static_cast<std::uint8_t>(PressedKeysStatus & (~Key_Up));
	...
	void Program::VideoResize(const std::shared_ptr<void> &sender, const std::shared_ptr<VideoResizeEventArgs> &e)
	{
		Size win = System::Windows::Forms::Screen::PrimaryScreen::WorkingArea::Size;
	...
	void Program::SaveSettings()
	{
		try
		{
			std::shared_ptr<BinaryWriter> f = std::make_shared<BinaryWriter>(File::Create(Environment::GetFolderPath(Environment::SpecialFolder::ApplicationData) + LR"(\blade_project.settings)"));
			f->Write(current_save_version);
			for (std::int32_t i = 0; i < 8; i++)
			{
	...
	void Program::LoadSettings()
	{
		...
		try
		{
			std::shared_ptr<BinaryReader> f = std::make_shared<BinaryReader>(File::Open(Environment::GetFolderPath(Environment::SpecialFolder::ApplicationData) + LR"(\blade_project.settings)", FileMode::Open));
			std::wstring ver = f->ReadString();
			if (ver != current_save_version)
			{
				f->Close();
	...
	void Program::InitRace()
	{
		...
		race_objects = std::vector<std::int32_t>(32);
		...
		hero_object->Animations = FindObjectAnimation(L"Race");
		hero_object->Life = 77;
		InitAnimation(hero_object, L"Ride", true);
	...
	void Program::DrawRaceTop()
	{
		if (top_x >= 0)
		{
			DrawScreenSprite(race_imgs, top_name + std::to_wstring(top_fase), top_x, 2, 0, 0, 32 - top_x, 8);
			DrawScreenSprite(race_imgs, L"back_top", 0, 2, 32 - top_x, 0, top_x, 8);
		}
		else
		{
			DrawScreenSprite(race_imgs, top_name + std::to_wstring(top_fase), 0, 2, -top_x, 0, 32 + top_x, 8);
			DrawScreenSprite(race_imgs, L"back_top", 32 + top_x, 2, 0, 0, -top_x, 8);
		}
	}
				...
				if (obst != 0)
				{
					...
					if (dist == 11 && hero_object->Life != 0 && bike_y_offset == 0 && bike_y_pos >= 184 && bike_y_pos < 192)
					{
						...
						if ((obst & pos) != 0)
						{
							...
							{
								bike_y_accel = -7;
								InitAnimation(hero_object, L"Jump", false);
								hero_object->NoInterruptAnim = true;
	...
	void Program::InitScene(std::int32_t num)
	{
		...
		LoadTexts();
		game_mode = clip_mode;
		std::wstring scene_name = L"";
		pictogramm_y = 10;
		switch (scene_num)
		{
			case 1:
				scene_name = L"j_escape";
				pictogramm_y = 17;
				break;
			case 2:
				scene_name = L"escape_1";
				break;
			case 3:
				scene_name = L"escape_2";
				break;
			case 4:
				scene_name = L"escape_3";
				break;
			case 5:
				scene_name = L"intro";
				pictogramm_y = 18;
			...
			case 7:
			case 8:
				scene_name = L"final_boss";
				pictogramm_y = 18;
				break;
			case 9:
				scene_name = L"finalcut";
				pictogramm_y = 18;
				break;
		}
	...
	std::shared_ptr<ScreensProject> Program::InitScreensProject(const std::wstring &name)
	{
		std::int32_t len = 9;
		if (tiles_mode == 0)
		{
			name = L"classic_" + name;
		}
		else
		{
			name = L"next_" + name;
		...
		std::shared_ptr<Stream> str;
		std::wstring path = LR"(../../../../../res/screens/)" + name + L".smproj";
		if (external_resource)
		{
			str = File::Open(path, FileMode::Open);
		}
		...
		std::shared_ptr<BinaryReader> proj = std::make_shared<BinaryReader>(str);
...
//C# TO C++ CONVERTER NOTE: The following call to the 'RectangularVectors' helper class reproduces the rectangular array initialization that is automatic in C#:
//ORIGINAL LINE: spr.tiles = new Tile[spr.width, spr.height];
			spr->tiles = RectangularVectors::RectangularTileVector(spr->width, spr->height);
			for (std::int32_t y = 0; y < spr->height; y++)
			{
				for (std::int32_t x = 0; x < spr->width; x++)
				{
					spr->tiles[x][y] = obj->Tileset[proj->ReadInt32()];
				}
			}
	...
	std::shared_ptr<Tile> Program::AddTile(const std::shared_ptr<ScreensProject> &obj, std::vector<std::uint8_t> &data)
	{
		std::int32_t len = 9;
		if (tiles_mode != 0)
		{
	...
	Color Program::GetAttrColor(std::uint8_t attr, bool enable)
	{
	...
	std::shared_ptr<Sprite> Program::FindSprite(const std::shared_ptr<ScreensProject> &proj, const std::wstring &name)
	{
	...
	std::shared_ptr<Screen> Program::FindScreen(const std::shared_ptr<ScreensProject> &proj, const std::wstring &name)
	{
		for (auto s : proj->Screens)
		{
			if (s->name == name)
			{
				return s;
	...
	void Program::DrawScreenSprite(const std::shared_ptr<ScreensProject> &proj, const std::wstring &name, std::int32_t xcord, std::int32_t ycord, std::int32_t xwin, std::int32_t ywin, std::int32_t width, std::int32_t height)
	{
	...
	void Program::SwitchSuite(bool hard_enable)
	{
		...
		if (prev_suit != current_suit || hard_enable)
		{
			...
			{
				hero_object->Animations = FindObjectAnimation(L"Suit");
			}
	...
	{
		SFX_Bank = std::vector<std::shared_ptr<Stream>>(32);
		SFX_Priority = std::vector<std::int32_t>(32);

		LoadSFX(LR"(../../../../../SFX/sfx_beetle.wav)", sfx_beetle, high_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_bumerang.wav)", sfx_bumerang, high_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_damage.wav)", sfx_damage, normal_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_dogscream.wav)", sfx_dogscream, high_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_dooropen.wav)", sfx_dooropen, high_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_doors.wav)", sfx_doors, normal_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_droid.wav)", sfx_droid, normal_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_elevator.wav)", sfx_elevator, high_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_explosion.wav)", sfx_explosion, high_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_gotonthewall.wav)", sfx_gotonthewall, high_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_lowpower.wav)", sfx_lowpower, high_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_missshoot.wav)", sfx_missshoot, high_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_powerup.wav)", sfx_powerup, high_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_race_block_hit.wav)", sfx_race_block_hit, high_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_race_down.wav)", sfx_race_down, high_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_race_jump.wav)", sfx_race_jump, high_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_race_wall_hit.wav)", sfx_race_wall_hit, high_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_rocket.wav)", sfx_rocket, high_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_shocker.wav)", sfx_shocker, high_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_slide.wav)", sfx_slide, high_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_slime.wav)", sfx_slime, high_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_start.wav)", sfx_start, high_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_stealth.wav)", sfx_stealth, normal_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_steam.wav)", sfx_steam, normal_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_stungun.wav)", sfx_stungun, high_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_switch.wav)", sfx_switch, high_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_tanketteborn.wav)", sfx_tanketteborn, normal_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_tanketteshoot.wav)", sfx_tanketteshoot, normal_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_terminal.wav)", sfx_terminal, normal_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_type.wav)", sfx_type, low_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_wave.wav)", sfx_wave, high_priority);
		LoadSFX(LR"(../../../../../SFX/sfx_star.wav)", sfx_star, high_priority);
	}

	void Program::LoadSFX(const std::wstring &name, std::int32_t num, std::int32_t priority)
	{
		SFX_Priority[num] = priority;
		if (File::Exists(name))
		{
			SFX_Bank[num] = std::make_shared<MemoryStream>(File::ReadAllBytes(name));
		}
		else
		{
			std::shared_ptr<Stream> str = std::static_pointer_cast<Stream>(Properties->Resources::getResourceManager()->GetObject(Path::GetFileNameWithoutExtension(name)));
			std::vector<std::uint8_t> ba(str->Length);
			str->Read(ba, 0, static_cast<std::int32_t>(str->Length));
			SFX_Bank[num] = std::make_shared<MemoryStream>(ba, 0, ba.size());
		}
	...
	void Program::InitSFX(std::int32_t num)
	{
		...
		if (sfx_busy == 0 || SFX_Priority[num] >= current_sfx_priority)
		{
			current_sfx_priority = SFX_Priority[num];
			sfx_busy = 20;

			//Stream stream = new MemoryStream(SFX_Bank[num]);
			SFX_Bank[num]->Position = 0;
			SFX_Player->Stream = SFX_Bank[num];
			SFX_Player->Play();
		}
	}
	...
	void Program::InitGameMusic(std::int32_t num, bool looped)
	{
		...
		std::wstring name = music_names[num];
		if (sound_mode == ts_mode)
		{
			name += L"_ts";
		}
		std::wstring path = LR"(..\..\..\..\..\music\oggs\)" + name + L".ogg";
		...
		else
		{
			...
			f = static_cast<std::vector<std::uint8_t>>(Properties->Resources::getResourceManager()->GetObject(name));
		}

		GameSoundTheme = std::make_shared<Music>(f);
		MusicPlayer::CurrentMusic = GameSoundTheme;
		MusicPlayer::Play(looped);
	}

	void Program::MuteGameMusic()
	{
		if (MusicPlayer::CurrentMusic != nullptr)
		{
			MusicPlayer::Stop();
		}
	}
	...
	void Program::InitMenuMusic(std::int32_t num, bool looped)
	{
		...
		MenuSoundTheme = std::make_shared<Sound>(f);
	...
	void Program::SpawnObjects(const std::shared_ptr<GameObject> &obj)
	{
		...
		if (obj->Name != 0 && obj->Name != idNova && obj->XYRoom == current_xy_room && ((obj->X1Trigger < hero_object->X + 1 && obj->X2Trigger >= hero_object->X && obj->Y1Trigger < hero_object->Y + 1 && obj->Y2Trigger >= hero_object->Y) || (obj->X1Trigger == -1 && cntr_enter_in_room != 0)))
		{
			...
			{
				case idPlatform:
					obj->Animations = FindObjectAnimation(L"Items");
					anim = L"Platform_Idle";
					obj->Fixed = true;
					break;
				case idSteam:
					obj->Animations = FindObjectAnimation(L"Items");
				...
				case idTankette:
					obj->Animations = FindObjectAnimation(L"Tankette");
					if (!InCurrentRoom(obj))
					{
						anim = L"Tankette_Move";
					}
					else
					{
						anim = L"Tankette_Uprise";
						obj->NoInterruptAnim = true;
					...
					break;
				case idDron:
					obj->Animations = FindObjectAnimation(L"Dron");
					if (!InCurrentRoom(obj))
					{
						anim = L"Dron_Move";
						obj->Status = 1;
				...
				case idBlower:
					...
					break;
				case idSpider:
					obj->Animations = FindObjectAnimation(L"Spider");
					anim = L"Spider_Run";
					obj->isDamageable = true;
				...
				case idBigPiranha:
					obj->Life = 1;
					obj->Animations = FindObjectAnimation(L"Piranha");
					anim = L"SmallPiranha_Swim";
					if (obj->Name == idBigPiranha)
					{
						anim = L"Piranha_Swim";
						obj->Life++;
					...
					break;
				case idJelly:
					obj->Animations = FindObjectAnimation(L"Jelly");
					anim = L"Jelly_Idle";
					obj->isDamageable = true;
					obj->Counter = 100;
					obj->Life = 2;
					break;
				case idBug:
					obj->Animations = FindObjectAnimation(L"Bug");
					if (!InCurrentRoom(obj))
					{
						anim = L"Bug_Run";
						obj->Status = 1;
					...
					break;
				case idDronBomber:
					obj->Animations = FindObjectAnimation(L"DronBomber");
					anim = L"DronBomber_Fly";
					obj->isDamageable = true;
					obj->Life = 1;
					obj->haveGravity = false;
					...
					break;
				case idGun:
					obj->Animations = FindObjectAnimation(L"Gun");
					anim = L"Gun_Born";
					obj->NoInterruptAnim = true;
					obj->isDamageable = true;
					obj->Fixed = true;
					...
					break;
				case idDragonfly:
					obj->Animations = FindObjectAnimation(L"Dragonfly");
					anim = L"Dragonfly_Fly";
					obj->isDamageable = true;
					obj->haveGravity = false;
					obj->Life = 1;
					break;
				case idFighterBomber:
					obj->Animations = FindObjectAnimation(L"FighterBomber");
					anim = L"FighterBomber_Fly";
					obj->isDamageable = true;
					obj->haveGravity = false;
					obj->Life = 2;
				...
				case idAlienClawBottom:
				case idAlienClawTop:
					obj->Animations = FindObjectAnimation(L"AlienClaw");
					anim = L"Claw_Bottom";
				...
				case idAlienWorm:
					obj->Status = 0;
					obj->Animations = FindObjectAnimation(L"AlienWorm");
					anim = L"AlienWorm_Born";
					obj->isDamageable = true;
					break;
				case idSlime:
					obj->Animations = FindObjectAnimation(L"Slime");
					anim = L"Walk2";
					obj->isDamageable = true;
					obj->Life = 2;
					obj->haveGravity = true;
					break;
				case idSonusVenator:
					obj->Animations = FindObjectAnimation(L"SonusVenator");
					anim = L"Venator_Idle";
					obj->isDamageable = true;
					obj->Life = 3;
					obj->haveGravity = true;
				...
				case idBigSpider:
					boss_mode = true;
					obj->Animations = FindObjectAnimation(L"BigSpider");
					anim = L"BigSpider_Idle";
					obj->isDamageable = true;
					obj->Life = 77;
					hero_path = 77;
					obj->Counter = 100;
				...
				case idCraken:
					boss_mode = true;
					obj->Animations = FindObjectAnimation(L"Craken");
					anim = L"Craken_Idle";
					obj->isDamageable = true;
					obj->Life = 77;
					hero_path = 77;
				...
				case idMoth:
					boss_mode = true;
					obj->Animations = FindObjectAnimation(L"Moth");
					anim = L"Moth_Idle";
					obj->Priority = low_priority;
					obj->Life = 77;
					hero_path = 77;
				...
				case idRocketman:
					boss_mode = true;
					obj->Animations = FindObjectAnimation(L"Rocketman");
					anim = L"Rocketman_Idle";
					obj->isDamageable = true;
					obj->Status = 1;
					obj->Life = 77;
					hero_path = 77;
				...
				case idCrabBoss:
					boss_mode = true;
					obj->Animations = FindObjectAnimation(L"CrabBoss");
					anim = L"Crab_Idle";
					obj->isDamageable = true;
					obj->Life = 77;
					hero_path = 77;
				...
				case idFlyBoss:
					boss_mode = true;
					obj->Animations = FindObjectAnimation(L"FlyBoss");
					anim = L"FlyBoss_Idle";
					obj->isDamageable = true;
					obj->Life = 77;
					hero_path = 77;
				...
				case idAlienPet:
					boss_mode = true;
					obj->Animations = FindObjectAnimation(L"AlienPet");
					anim = L"Pet_Idle";
					obj->isDamageable = true;
					obj->Life = 77;
					hero_path = 77;
				...
				case idAlienSliderman:
					boss_mode = true;
					obj->Animations = FindObjectAnimation(L"AlienSliderman");
					anim = L"Slider_Idle";
					obj->isDamageable = true;
					obj->Life = 77;
					hero_path = 77;
				...
				case idMegabrain:
					boss_mode = true;
					obj->Animations = FindObjectAnimation(L"Megabrain");
					anim = L"Megabrain_0";
					obj->Priority = low_priority;
					obj->isDamageable = true;
					obj->Life = 77;
				...
				case idDragon:
					boss_mode = true;
					obj->Animations = FindObjectAnimation(L"DLCBoss");
					anim = L"Idle";
					obj->isDamageable = true;
					obj->Life = 77;
					hero_path = 77;
	...
	void Program::InitStealth()
	{
		...
		ObjectAnimations.clear();
		LoadAnimations(L"Jacqueline");

		hero_object = CreateObject(idJacqueline, 0, 0, false, true);
		hero_object->Animations = FindObjectAnimation(L"Jacqueline");
		InitAnimation(hero_object, L"J_Idle0", true);
		hero_object->Priority = normal_priority;
		lift_moves = 0;
		next_lift_num = 1;
		j_game_over = false;
		...
		if (lift_moves != 0 && !lift_object->AnimationPlayed)
		{
			...
			if (lift_moves == 0)
			{
				...
				for (auto obj : actual_objects)
				{
					...
					{
						StealthSpawn(obj);
						std::wstring anim = L"JLift_FinishUp";
						if (lift_object->Counter < 128)
						{
							anim = L"JLift_FinishDown";
						}
			...
			if ((game_cicle_counter & 31) == 0)
			{
				if (hero_energy != 0)
				{
					...
					if (hero_energy == 0)
					{
						...
						if (!obj->NoVisible)
						{
							InitJAnimation(obj, L"J_Invisible", true);
							obj->NoInterruptAnim = true;
							InitSFX(sfx_stealth);
						}
						return;
	...
	void Program::JPolice(const std::shared_ptr<GameObject> &obj)
	{
		...
		std::wstring anim = L"Police1_Idle";
		std::int32_t d = 32;
		...
		{
			anim = L"Police1_Attack";
			if (obj->Name == idJPolice2)
			{
				anim = L"Police2_Attack";
			}
			if (obj->Name == idJPolice3)
			{
				anim = L"Police3_Attack";
			}
			hard_anim = true;
			obj->NoInterruptAnim = true;
			JOfficePanic();
	...
	void Program::JOffice(const std::shared_ptr<GameObject> &obj)
	{
		...
		std::wstring anim = L"Office1_Idle";
	...
	void Program::JLift(const std::shared_ptr<GameObject> &obj)
	{
		if (lift_pause != 0)
		{
			...
			if (lift_pause == 0)
			{
				InitJAnimation(obj, L"JLift_FinishUp", true);
			}
		}
			...
			if (GetKey(Key_Up))
			{
				...
				if (dist < 16)
				{
					...
					else
					{
						...
						lift_object = obj;
						std::wstring anim = L"JLift_StartUp";
						if (obj->Counter < 128)
						{
							anim = L"JLift_StartDown";
						}
						InitJAnimation(obj, anim, true);
			...
			if (!hero_object->NoVisible && !hero_object->NoInterruptAnim)
			{
				...
				if (d < 16)
				{
					...
					else
					{
						...
						{
							InitJAnimation(hero_object, L"JHack", false);
						}
						else
						{
							InitJAnimation(obj, L"JDoor_Open", true);
							hero_object->NoVisible = true;
	...
	std::shared_ptr<GameObject> Program::FindDoor()
	{
		for (auto obj : actual_objects)
		{
			if (obj->Name == 0)
	...
	void Program::JTerminal(const std::shared_ptr<GameObject> &obj)
	{
		...
		else
		{
			...
			if (GetKey(Key_Up) && d < 16)
			{
				...
				{
					hero_object->NoInterruptAnim = true;
					InitJAnimation(hero_object, L"J_Terminal", true);
					if (terminal_num != 0)
					{
	...
	void Program::InitJAnimation(const std::shared_ptr<GameObject> &obj, const std::wstring &anim, bool hard)
	{
		if (!hero_invisible)
		{
			anim += L"_0";
	...
	void Program::StealthSpawn(const std::shared_ptr<GameObject> &obj)
	{
		if (obj->Name != 0 && obj->XYRoom == current_xy_room)
		{
			...
			obj->NoActivated = false;
			obj->NoInterruptAnim = false;
			obj->Animations = FindObjectAnimation(L"Jacqueline");
			obj->Life = 1;
			switch (obj->Name)
			{
				case idJLift:
	...
	{
		std::wstring path = LR"(../../../../../res/fonts/font_)" + StringHelper::toLower(language) + L".ch8";
		std::vector<std::uint8_t> f;
		if (external_resource)
		{
			f = File::ReadAllBytes(path);
			next_mini_powerups = std::make_shared<Surface>(LR"(../../../../../res/fonts/platformer_mode_font.png)");
		}
		else
		{
			f = static_cast<std::vector<std::uint8_t>>(Properties->Resources::getResourceManager()->GetObject(L"font_" + StringHelper::toLower(language)));
			next_mini_powerups = std::make_shared<Surface>(Properties->Resources::getPlatformerModeFont());
		}
		...
		{
//C# TO C++ CONVERTER NOTE: The following call to the 'RectangularVectors' helper class reproduces the rectangular array initialization that is automatic in C#:
//ORIGINAL LINE: font32[j] = new bool[8, 8];
			font32[j] = RectangularVectors::RectangularBoolVector(8, 8);
			for (std::int32_t i = 0; i < 8; i++)
			{
				std::int32_t a = f[n];
				n++;
	...
	void Program::PrintString32(std::int32_t x, std::int32_t y, const std::wstring &text)
	{
	...
	{
		InitFonts();
		std::wstring path = LR"(../../../../../res/)" + language + L".bin";
		if (File::Exists(path))
		{
			texts = File::ReadAllBytes(path);
		}
		else
		{
			texts = static_cast<std::vector<std::uint8_t>>(Properties->Resources::getResourceManager()->GetObject(language));
		}
	}
	...
	void Program::ScrollWindow(std::int32_t x, std::int32_t y, std::int32_t width, std::int32_t height)
	{
		...
		screen->SetPixels(Point(x, y), screen->GetColors(Rectangle(x, y + 8, width, height - 8)));
		screen->Fill(Rectangle(x, y + height - 8, width, 8), Color::Black);
	}
	...
	void Program::ScrollCredits()
	{
		...
		screen->SetPixels(Point(x, y), screen->GetColors(Rectangle(x, y + 1, width, height - 1)));
		screen->Fill(Rectangle(x, y + height - 1, width, 1), Color::Black);
	}
}
